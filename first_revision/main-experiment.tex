...
\revision{TODO WHAT EXACTLY TO WRITE? WHERE TO PUT THIS? In Figure~\ref{fig:median-table-expt2}, the difference in memory capacity between real and baseline orders, as provided by our bound, is in the range of at most a few bits.
...
%	This suggests that the weaker optimization in free word order languages observed in Experiment 2 might in part be because ordering grammars did not take information structure into account.\jd{i still think that instead of copying the whole figure, it would be better to just show fig 13 with both versions of czech, where color distinguishes the two versions (labeled in legend). but up to you; i imagine reviewers will make you do this eventually anyway}
