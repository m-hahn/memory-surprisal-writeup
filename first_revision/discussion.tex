...
\revision{For discussion including mathematical proofs, see SI Section 1.4.}
\revision{Depending on the precise formalization of the production problem, the production-oriented version of the memory--surprisal tradeoff may or may not be identical the comprehension-oriented version we have presented here. 
...
\revision{We specifically expect this to happen in languages where less data is available (see SI Section 3.1 for corpus sizes).}
...
\revision{To the extent that $I_t$ is underestimated even for small values of $t$, such a bias equally applies to different ordering grammars. We therefore expect that estimating the relative efficiency of different orderings at the same level of memory is still reliable.}
