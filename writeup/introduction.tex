...
%Natural language solves the problem of allowing humans to communicate effectively given the limited cognitive resources available for producing and understanding utterances.\jd{whether natural language provides a vs the solution, and whether the problem that is solved is one of simply communicating vs communicating effectively, is sth that's up for debate. the way it sounds here is that we're taking as given that natural language is optimized for effective communication under resource limitations. but surely that's what we're testing, rather than what we want to presuppose? i worry that this initial sentence will give the whole paper a question-begging flavor. how about sth like: "Natural language is a powerful tool that allows humans, with our inherently limited cognitive resources, to communicate."} 
...
We formalize the notion of memory constraints in terms of what we call the \key{memory--surprisal trade-off}: the idea that the ease of comprehension %\jd{still don't know what "accuracy of production" is supposed to mean, and i imagine this formulation will trip up readers. we can talk about what a more intuitive short phrase would be to capture what is intended here}
depend on the amount of computational resources invested into remembering previous linguistic elements, e.g., words. %\jd{instead of "words", should this be "linguistic elements, e.g., words"?}
...
Within our framework, we prove a theorem showing that lower memory requirements result when utterance elements that depend on each other statistically are placed close to each other. This theorem does not require any assumptions about the architecture or functioning of memory, except that it has a bounded capacity.
