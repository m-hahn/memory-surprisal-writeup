...
The x-axis indicates nats of memory, i.e. computing logarithms using base e.
	\mhahn{We might change it to bits, though changing it throughout the entire paper \& SI seems like a bigger task} \jd{it's typically easier for people to think in bits than nats ("people" being me), so if possible, changing it in all the figures might be a useful time investment. or else keep it at nats, but be clear in axis labels about what is displayed -- somehow the reviewer came away with the idea that it's bits that are displayed, probably because the text consistently makes reference to bits (and in fact fig 1 explicitly says bits on the x-axis). so actually, thinking through this: either change all the text or change all the figures. changing text is faster but changing figures will make it easier on readers.}
	...
	\mhahn{do we have to say something in the paper?}\jd{yes, add a note to this effect! this will be confusing to many readers}
\end{reply}
...
\jd{not sure what you have in mind here. it's not clear to me that there would be an obvious bias in the results -- should there be? though i think it would be useful to include some kind of visualization of dataset size, either directly on the plot (perhaps just by including training data size in fig 10) or by including a table, and point out that the larger the dataset, the more trustworthy the result is? }
...
We believe that taking in time is necessary to derive the stronger bound $\sum_{t=1}^T t I_t$. \jd{nice. ``we believe that" is pretty weak -- either it's necessary or not. but more generally: what the reviewer is pointing out is that it might help people to start out with a time-free version to build intuitions, then add time. looking back at the manuscript, it's not clear to me that this is easily done, but perhaps sth to think about?}
\end{reply}
...
\begin{reply}
\jd{``We agree that Study 2 is stronger than Study 1. But Study 1 serves as an illustration of the theoretical results in a simpler, controlled but language-like setting, which we hope allows the reader to build intuitions about the application of the theorem. We have thus chosen to keep the order.''}We chose to keep the order, as we believe Study 1 can serve as an illustration of the theoretical results in a controlled but language-like setting.
...
\begin{reply}
	TODO think about this \jd{given that the reviewer has zero recommendations about which parts of the methods they found superfluous, i'm not sure it's worth doing this. reading through the methods again, it's not at all obvious what should be moved to the SI}
\end{reply}
...
\begin{reply}
	TODO think about this \jd{isn't this precisely what section 5.5 does? perhaps i misunderstand the reviewer's point}
\end{reply}
...
\begin{reply}
	TODO maybe cite these more prominently? maybe cite them in ``Architectural Assumptions'' \jd{that sounds like a good place for them}
\end{reply}
...
\begin{reply}
	...
	%\mhahn{We should point ut that info locality makes predictions beyond DLM, in morpheme ordering and adjective ordering. Where do we do this in the paper?}
\end{reply}
...
\begin{reply}
	We now include a section `Relation to linguistic typology' in the General Discussion to address the Reviewer's comment. \jd{this is a great new section}
\end{reply}
...
\begin{reply}
	We now include a section `Relation to linguistic typology' in the General Discussion to address the Reviewer's comment and why our model is interesting to questions in linguistic typology.\jd{what about the other questions the reviewer asks?}
\end{reply}
