...
In the original version, the x-axis indicated \emph{nats} of memory, that is, all logarithms were computed using base $e$.
	We have now changed all values to \emph{bits} throughout the paper and the SI, i.e. computing logarithms using base $2$ (one \emph{nat} equals about $1.44$ \emph{bits}).
...
\begin{reply}
	We now include a section `Relation to linguistic typology' in the General Discussion to address the Reviewer's comment, including the relation to dependency locality and why our model is interesting to questions in linguistic typology.\jd{what about the other questions the reviewer asks?}
\end{reply}
