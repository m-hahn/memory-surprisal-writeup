...
The \key{memory--surprisal trade-off curve} answers the question:\jdegen{move this right after the question}
...
\includegraphics[width=0.4\textwidth]{figures-gdrive/mi-distance.pdf}
	(b)
\includegraphics[width=0.2\textwidth]{figures-gdrive/theorem.pdf}
	\caption{
		...
		(c) \jd{FILL OUT} \note{make the colors in (c) difefrenfr from this in (b). Do want to make clear what the mapping from (b) to (c) is. Maybe can even have a pair of the two (b) curves, and then show what they map on. \mhahn{keep but redo in vector graphics}}
}\label{fig:theorem}
\end{figure}
