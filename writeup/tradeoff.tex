...
If the comprehender utilizes an average of $I_t$ bits of information coming from $X_{t_0-t}$ in predicting $X_{t_0}$, then these bits must have been carried over $t$ timesteps, and thus have occupied memory for $t$ timesteps.
Since this happens for every word in a sequence, there are, for any given point in time, $t$ such packages of information with an average size of $I_t$ bits each that have to be carried across it, summing up to $t I_t$.
