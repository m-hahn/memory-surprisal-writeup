...
generating a stream of symbol extending indefinitely into the past and future: $\dots, w_{-2}, w_{-1}, w_0, w_{1}, w_{2}, \dots$. %indexed as $w_1, \dots, w_t, \dots$.
These symbols can represent words, morphemes, or other units for decomposing sentences into a sequence of symbols. 
We model this process as \emph{stationary} \CITE, that is, the joint distributions of symbols at different time points depend only on their relative positions in time, not their absolute positions (see SI Section \REF for more on this modeling assumption).
...
\begin{equation}
   S_M \equiv \sum_{w_t,m_t} \cdot P(m_t) P(w_t|m_t) \left( -\log P(w_t | m_t) \right).
\end{equation}
...
%\end{equation}
Because the process $W$ is stationary, the average surprisal $S_M$ is independent of the choice of $t$ (see SI Section \REF).
...
Again, because $W$ is stationary, this quantity does not depend on the choice of $t$ (see SI Section \REF).
