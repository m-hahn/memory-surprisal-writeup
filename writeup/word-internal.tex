...
%Tense/aspect & Passive & 302 \\ % this one also arises for the fitted grammar
...
\end{tabular}
    ...
    %We exclude divergences in those cases where they are also predicted incorrectly by the grammar fitted to the orders.
    %We only count divergences as errors here if they are predicted by the order (TODO figure out where exceptions come from). Also, w
    We only show errors where both morphemes are among the high-frequency ones studied here.}
