...
We found that ordering morphemes according to these fitted models recovered the observed order for almost all forms (XX\% for Japanese, XX\% for Sesotho prefixes, XX\% for Sesotho suffixes).
...
%causative & suru & 2 \\
\end{tabular}
    \caption{Errors in Japanese. We only count divergences as errors here if they are predicted by the order (TODO figure out where exceptions come from). Also, we only show errors where both morphemes are among the high-frequency ones studied here. \rljf{How is the frequency column calculated?}}
...
The only divergence among the high-frequency morphemes is that negation and the tense/aspect prefix are ordered incorrectly; this accounts for only 11 occurrences in the data set, as the two prefixes rarely co-occur.
...
\end{tabular}
    \caption{Errors in Sesotho prefixes. We only count divergences as errors here if they are predicted by the order (TODO figure out where exceptions come from). Also, we only show errors where both morphemes are among the high-frequency ones studied here.}
...
\end{tabular}
    \caption{Top 5 Errors in Sesotho suffixes. We only count divergences as errors here if they are predicted by the order (TODO figure out where exceptions come from). Also, we only show errors where both morphemes are among the high-frequency ones studied here.}
    \label{tab:sesotho-suffix-err-analysis}
\end{table}
...
\begin{quote}
verb-valence-voice-aspect-tense-mood-modality-subj.person-subj.number \rljf{Make pretty}
\end{quote}
Morphemes are claimed either to go in the order above, or its reverse.
