...
There is the possibility of a confound in the fact that baselines are constructed using a grammar formalism that is in some ways too restrictive to model all word order rules found in natural languages.
...
\mhahn{fill in}
We show estimated tradeoffs in Figure~\ref{fig:median-table}.

\mhahn{add AUC figures for consistency with Experiment 3}
...
A smaller AUC value indicates a more efficient tradeoff; in most cases, the real orders provide more efficient tradeoffs than most or all baseline grammars.\mhahn{TODO make colors consistent with previous figure}
}\label{fig:auc-mle}
\end{figure}
...
\mhahn{show AUC figures}

TODO talk about Figures ~\ref{fig:median-czech-infostruc}, \ref{fig:freedom-mi-with-infostruc}

In Figure~\ref{fig:freedom-mi-with-infostruc}, we provide a version of figure (REF) showing how the data point for Czech would move when modeling word order including information structure.
...
\includegraphics[width=0.5\textwidth]{figures/Czech-PDT-listener-surprisal-memory-MEDIANS_onlyWordForms_boundedVocab.pdf}
	\caption{Memory-surprisal tradeoff Czech with information structure. Green: Baselines with information structure. Red: Baselines without information structure. Blue: Real \mhahn{add AUC plot}}\label{fig:median-czech-infostruc}
\end{figure}
