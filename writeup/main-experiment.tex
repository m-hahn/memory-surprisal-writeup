...
%\jd{be more specific, eg: "Measuring a language's memory--surprisal tradeoff from corpus data requires estimating quantities x, y, z, which can then be inserted into Theorem 2 to yield a lower bound on the tradeoff."}
These quantities can be plugged in to Theorem~\ref{prop:suboptimal} to give a lower bound on the memory--surprisal trade-off. \jd{throughout the document, ultimately make sure to commit to either the tradeoff or trade-off spelling}
