...
Here, we describe how we measure the memory--surprisal trade-off in corpora, and how we generate counterfactual baseline languages. In Section~\ref{sec:main-experiment-results}, we compare the trade-off in real corpora against the trade-off in the counterfactual baselines. For the majority of languages, we find that the real languages have more favorable memory--surprisal trade-offs than the baselines, in line with our Main Hypothesis\jd{again, i'd relabel the main hypothesis}.

\subsection{Measuring the memory--surprisal trade-off in corpora}

The key to evaluating the memory-surprisal tradeoff from corpus data is the set of quantities  \jd{instead of "the set of quantities", say in prose what those quantities correspond to} $I_t$ (defined in Section~\ref{sec:infoloc}), the  mutual information between words at distance $t$ conditional on the intervening words. \jd{be more specific, eg: "Measuring a language's memory--surprisal tradeoff from corpus data requires estimating quantities x, y, z, which can then be inserted into Theorem 2 to yield a lower bound on the tradeoff."}
All that is required is to estimate the quantities $I_t$; then these can be plugged in to Theorem~\ref{prop:suboptimal} to give a lower bound on the memory--surprisal trade-off. \jd{throughout the document, ultimately make sure to commit to eithe rthe tradeoff or trade-off spelling}
...
See Supplementary Materials Section \jd{X} for details on how these models were fit to data, and see Supplementary Materials Sections \jd{Y} and \jd{Z} for control studies using other methods of estimating $I_t$ (based on $n$-gram models and PCFG chart parsers). The results of these control studies are in agreement with the neural network results presented in this section.\jd{can sth less vague be said here, eg "These control studies yield the same qualitative results as the neural network-based studies presented here"}

In order to evaluate the average surprisal values $S_t$, we computed the empirical word-by-word surprisal values under the $t$th-order Markov model for held-out data, different from the data that was used to train the model. By evaluating on held-out data, we avoid underestimating the value of $S_t$ due to overfitting.\jd{say a tiny bit more about the model fitting procedure, eg how much data was held out}
...
We excluded data from historical languages.\footnote{Historical languages excluded: Ancient Greek, Classical Chinese, Coptic, Gothic, Latin, Old Church Slavonic, Old French.}\jd{add half a sentence on why?}
...
We provide the sizes of the resulting datasets in SI Table (\REF).\jd{can you give a one-sentence descriptive summary of dataset sizes here to give people an idea? eg, these datasets ranged in size from X to Y}

\mhahn{say at some point how we're arranging sentences into a stationary stochastic process}\jd{yes}

\subsection{Defining baselines}

Test the \jd{Main Hypothesis} requires comparing the memory--surprisal tradeoffs of real grammars to those of baseline grammars. The baseline grammars we construct \jd{at the very end, make sure to either consistently use past or present tense for things that were done (i prefer past tense for reports of what was done, and present tense for conclusions, implications, generalizations)} are counterfactual ordering grammars that define consistent ordering rules similar to those found in actual languages (Figure~\ref{fig:grammars}).
...
Such baseline grammars define languages that have consistent word order but, crucial for our purposes, do not exhibit systematic preferences for specific word order patterns such as short dependencies. \jd{confusing rhetorical relation between previous and following sentence} In actual languages, the ordering of words is largely determined by syntactic relations (CITE).
...
See Sections~\ref{subsec:expt2-discussion} and \ref{subsec:freedom} for further discussion.\jd{this paragraph is a little confusing. is this supposed to convince the reader that ordering grammars are a good formalism within which to construct baseline grammars? if so, pre-empt the question about the effect this will have for free word order languages by explicitly mentioning the issue and deferring its discussion to later, rather than simply saying that word order freedom is not modeled (about which the uninitiated reader will wonder whether that's a good or a bad thing)}


We first constructed at least 10 baseline grammars for each of the 54 real languages.
We then continued to construct baseline grammars until a precision-based stopping criterion was reached (CITE). This criterion was designed to ensure that enough grammars were sampled to reliably compare the tradeoff curves of real and baseline grammars, without biasing results towards or against our hypothesis (see SI Section \REF). \jd{say a little more about the precision-based stopping criterion -- what was it, in one sentence? and add a one-sentence descriptive summary of the resulting number of baseline grammars per language, eg "the resulting number of baseline grammars ranged from X (LANGUAGE A, corpus size) to Y (LANGUAGE B, corpus size)}
...
In Figure~\ref{fig:median-table}\jd{why is table 14 referenced before tables 11-13?}, we show the resulting bounds on the memory-surprisal tradeoff curves, showing median surprisals at given levels of memory, for real and baseline languages.
We compute surprisal at 40 evenly spaced points of memory (selected individually for each language\jd{how?}), over real orders and baseline grammars.
...
This result could point to different explanations \jd{for what?}.
...
A possible concern with this experiment is that the optimization observed in Experiment 2 might be an artifact of the grammar formalism\jd{if we send this to cognition, i think we'll get some pushback on calling these studies "experiments", since the cognitive psychologists have clear ideas on what counts as an experiment, and running models doesn't count :) perhaps call them "studies" instead?}:
