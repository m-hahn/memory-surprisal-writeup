...
See Supplementary Materials Section \REF for details on how these models were fit to data, and see Supplementary Materials Sections \REF and \REF for control studies using other methods of estimating $I_t$ (based on $n$-gram models and PCFG chart parsers). These control studies yield the same qualitative results as the neural network-based studies presented here.
...
\includegraphics[width=0.95\textwidth]{figures/surprisal-branching-entropy-REAL-invert.pdf}
	...
	Languages with higher branching direction entropy show a smaller reduction in surprisal compared to baseline orders.\jd{is there sth to say about the outliers -- uyghur, arabic, french?} \mhahn{remove `Adap' from labels}
