...
\subsection{Role of Comprehension, Production, and Change}
...
\cite{fedzechkina2020production}

\mhahn{TODO reorganize this}

Our results leave open the causal mechanism leading to the observed optimization.

One option is that optimization reflects an effort on the side of the speaker to produce utterances that are easy to comprehend by listeners \citep{clark1982audience,lindblom1990communication, brennan1995feeling}.

Optimization may also reflect production-internal pressures to minimize effort on the speaker's part during sentence planning \citep{ferreira2000effect,macdonald2013language}.

Optimization may also reflect biases that come into play during language learning.
...
The language processing system might be set up to favor more efficient orders.

Our results are compatible with these different mechanisms.
While our theory is stated in terms of the efficiency of language processing for a comprehender of language, we can show that an analogous memory--surprisal trade-off exists in language production, and that speakers with bounded memory capacity can produce a language more accurately when it has stronger information locality (see SI Section 1.4 for a mathematical statement and proof).
...
What our results do imply, though, is that real languages provide lower surprisals in the setting of relatively small memory budgets. This result only depends on the estimates of $I_t$ for small values of $t$, which are most trustworthy.
...
We provide an artificial example with analytical calculations in SI Section 2.1; however, that example does not seem linguistically natural.
...
\paragraph{Connectionist Models}

\mhahn{also mention connectionist models of memory in incremental processing, which were in part framed as alternatives to memory capacity accounts (but actually fit well with our framework)? such as \cite{macdonald2002reassessing,christiansen2009usage}. also focus on experience-based mechanism \cite{wells2009experience}. can also mention \cite{rasmussen2018left} in this context}

\mhahn{at some point we can say the following. where? it could also go with discussion of experience-based models (like MacDonald and Christiansen) of memory in sentence processing}
